...
abstract class AbstractActor implements Actor {
    ...
    private final PropertyChangeSupport changeSup;
    ...
    protected void fireSentenceSaid(String s) { 
    	synchronized (LOCK) {
        	String phrase =fmt.format(s);
        	sentSup.fireSentenceSaid(phrase);
		}
    }
    ...
    private final class TaskCode implements Runnable {
        ...
        private void oneStep() {
            /*****************/
        	fireSentenceSaid("demande du verrou");
        	synchronized (LOCK){
        		fireSentenceSaid("obtention du verrou");
        		while (!thread.isInterrupted() && !canUseBox()){
        			fireSentenceSaid("Rendre dans la salle d'attente");
        			setActive(false);
        			try {
						LOCK.wait();
					} catch (InterruptedException e) {
